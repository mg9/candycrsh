...
public class startPoint {
	...
	public startPoint() {
		...
		final JRadioButton savedGame = new JRadioButton("Play Saved Game");
		c.fill = GridBagConstraints.HORIZONTAL;
		...
		bG.add(savedGame);
		final JRadioButton levelGame = new JRadioButton("Choose Level");
		c.fill = GridBagConstraints.HORIZONTAL;
		// c.ipady = 40; //make this component tall
		c.weightx = 0.5;
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				...
				if(levelGame.isSelected()){
					uA.loadLevel(Integer.parseInt(levelList.getSelectedItem().toString()));
					frame.setVisible(false);
					
				}
				if (savedGame.isSelected()) {
					String x=(String) levelListx.getSelectedItem();
				 try {
					uA.LoadGame(x);
